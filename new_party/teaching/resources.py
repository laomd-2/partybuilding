...
class CreditResource(resources.MyResource):
    ...
    def _group_to_dict(self, dataset):
        dataset1 = dict()
    ...
    def to_workbook(self, dataset):
        dataset = self._group_to_dict(dataset)
        ...
        for branch in sorted(dataset.keys()):
            ...
            for netid in sorted(member_takes.keys(),
                ...
                sheet.cell(i, 4, '正式党员' if member.is_real_party_member() else
                                ('预备党员' if member.is_pre_party_member() else '')).alignment = alignment
