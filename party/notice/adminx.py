...
from xadmin.views import IndexView, ListAdminView, BaseAdminPlugin
from .util import *
...
class FirstTalk(Table):
    ...
    @classmethod
    def get_update_fields(cls):
        return field_range('application_date', 'first_talk_date')
...
class Activist(Table):
    ...
    @classmethod
    def get_update_fields(cls):
        return field_range('application_date', 'activist_date')
...
class PreMember(Table):
    ...
    @classmethod
    def get_update_fields(cls):
        return field_range('key_develop_person_date', 'first_branch_conference')
...
class FullMember(Table):
    ...
    @classmethod
    def get_update_fields(cls):
        return field_range('first_branch_conference', 'second_branch_conference')
...
def get_graduation(request):
    ...
    fields = ['branch', 'netid', 'name', 'out_type', 'out_place']
    return fields, [m['netid'] for m in members.values('netid')]


class PlanPlugin(BaseAdminPlugin):
    ...
    def get_context(self, context):
        ...
        if is_admin(self.request.user):
            ...
            affairs.append(('毕业生组织关系', len(graduation),
                            ','.join(map(lambda x: str(x), graduation)), fields))
...
class AffairPostPlugin(BaseAdminPlugin):
    def init_request(self, *args, **kwargs):
        return True

    def get_list_queryset(self, query):
        netids = self.request.POST.get('netid__in')
        if netids:
            query = query.filter(netid__in=netids.split(','))
        return query


xadmin.site.register_plugin(PlanPlugin, IndexView)
xadmin.site.register_plugin(AffairPostPlugin, ListAdminView)
